...
**Case 1)** In  @eq-gpd-cdf, when $\left(1 + \dfrac{\xi (x-u)}{\sigma}\right) > 0$. As we can see from above, this happens when $\quad i) \xi >0$ or $\quad ii)$ from @eq-cond-neg-xi when $u -\dfrac{\sigma}{\xi} > x$.  Let $y = F(x;\,\sigma,\xi, u)$ then
...
#'
#' @returns Numeric vector quantile (or inverse cumulative distribution) corresponding to the probability p
#' @export
#'
#' @examples
