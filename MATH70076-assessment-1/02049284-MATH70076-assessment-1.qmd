...
\end{aligned}
$$
Hence, our well defined $F^{-1}_X: [0,1] \rightarrow [u, x^+]$ of $X \sim \text{GPD}(u, \sigma, \xi) is as follows:
...
qgpd <- function(p, xi, sigma, u){
  ...
  # "valid" version of p by setting any values outside the
  # valid [0, 1] range to NA. This effectively flags them as invalid
  # before any calculations are performed.
  p_valid <- ifelse(p < 0 | p > 1, NA, p)
  ...
  q <- ifelse(
    xi == 0,
    # Formula for xi = 0 
    u - sigma * log(1 - p_valid),
    # Formula for xi != 0 
    u - (sigma / xi) * (1 - (1 - p_valid)**(-xi) )
  )
...
print(paste("The quantile for p = 0.50 is:", quantile_p05))

# Quantile for p = 0.75
quantile_p75 <- qgpd(p = 0.75, u = 1.5, sigma = 2, xi = -0.4)
print(paste("The quantile for p = 0.75 is:", quantile_p75))

# Quantile for p = 0.99
quantile_p99 <- qgpd(p = 0.99, u = 1.5, sigma = 2, xi = -0.4)
print(paste("The quantile for p = 0.99 is:", quantile_p99))
