...
#| echo: false
#| warning: false
#| fig-cap: "Q–Q Plots for GPD Samples: Stated vs. Simulated Parameters"
#| layout-ncol: 2
...
for (current_id in set_ids) {
  ...
  # Using for loops, could use vectorisation, but this helps me be in control
  for (i in 1:n) {
...
# use qgpd function to find the corresponding theoretical quantiles for sim
  ...
  p <- ggplot(df) +
    geom_point(aes(x = Theoretical, y = Empirical, color = "Stated Params"), alpha = 0.8) +
    ...
    labs(
      ...
      y = "Empirical (Sample) Quantiles",
      color = ""
    ) +
    theme_minimal() +
    scale_color_manual(values = c("Stated Params" = "blue", "Sim Params" = "red"))
