...
format:
  ...
  pdf: default
format-links: false
bibliography: 02049284-MATH70076-assessment-1.bib 
---
```{r}
#| echo: false
#| warning: false
...
<!-- YOUR DERIVATION GOES HERE -->
 ...
 For this assessment in general we will be using R [@baseR] for coding purposes and libraries dplyr [@dplyr] and ggplot2 [@ggplot2ref] for plotting the various graphs.
 
To invert the cumulative distribution function of the GPD, to get $F^{-1}_X: [0,1] \rightarrow [u, x^+]$, we will start from @eq-gpd-cdf. We have to consider different cases. Firstly when $\xi \neq 0$ but $\left(1 + \dfrac{\xi (x-u)}{\sigma}\right) > 0$. Solving for when this happens. As $x \geq u$, if $\xi >0$, this is always the case. But even when $\xi < 0$, we can get a positive value for  $\left(1 + \dfrac{\xi (x-u)}{\sigma}\right) > 0$. This happens if (keeping in mind that $\xi < 0 \implies -\xi > 0$): 
...
**Case 1)** In  @eq-gpd-cdf, when $\left(1 + \dfrac{\xi (x-u)}{\sigma}\right) > 0$. As we can see from above, this happens when $i) \xi > 0$ or $ii)$ from @eq-cond-neg-xi when $u -\dfrac{\sigma}{\xi} > x$.  Let $y = F(x;\,\sigma,\xi, u)$ then
...
**Case 2)** when $\xi = 0$, we get the following from @eq-gpd-cdf, like above let $y = F(x;\,\sigma,\xi, u)$ then
...
```{r}
#' Quantile Function for the Generalized Pareto Distribution (GPD)
#'
#' Computes the quantile (inverse cumulative distribution) for a Generalized Pareto
#'
#' @param p  Numeric vector of probabilities, should be between 0 and 1 (inclusive)
#' @param xi Numeric, shape parameter
#' @param sigma Positive numeric, scale parameter
#' @param u Numeric, location parameter
#'
#' @returns Numeric vector quantile (or inverse cumulative distribution)
#'  corresponding to the probability p
#' @export
#'
#' @examples
#' # Quantiles for X ~ GPD(u = 1.5, sigma = 2, xi = -0.4)
#' qgpd(p = 0.5, xi = -0.4, sigma = 2, u = 1.5)
#' qgpd(p = c(0.25, 0.75, 0.99), xi = -0.4, sigma = 2, u = 1.5)
...
#| echo: false
#| warning: false
#| fig-cap: "Q–Q Plots for GPD Samples: Non_sim vs. sim Parameters along with y = x line."
#| layout-ncol: 2
...
**Proposal 1: ** To investigate Proposal 1 "the distribution of river flow values is constant over time", we will first create a time series plot to see the trend of riverflow.
...
From @fig-riverflow-ts, and from the csv file, we can see there are some extreme data (given from the column `is_extreme` in the csv file), so first checking that the number of extreme values are almost equal per year so that we are not adding bias to our investigation, by ignoring the extreme values.
...
As we can see from @fig-yearly-extremes that the number of extremes is almost similar so we can only consider non-extreme data. Now we will plot the non-extreme river flow data along with the baseline means, to see if there are any trends. And secondly, we will plot monthly box plots which will also contain only the non-extreme data, this will also help us visualise if there are any trends. 
...
@fig-riverflow-non-extreme clearly reveals a strong seasonal pattern. The data exhibits cyclical peaks and troughs that recur annually, suggesting that river flow is influenced by seasonal weather patterns. This directly contradicts the proposal that the distribution of river flow remains constant over time. The dotted red line represents the provided `baseline_mean`, which also shows a distinct seasonal trend and a slight upward drift over the years, further contradicting the hydrologist’s proposal. @fig-monthly-boxplot supports our counterargument that there is variation of the means of the river flow per month, as we can see from the boxplots that the means of river flow per months are not the same and there are two peaks per year (in May and a smaller one in November). In conclusion, the river flow is not constant over time, it exhibits a clear seasonal trend, and the mean appears to be gradually increasing. This indicates that both the variability and the average level of river flow change over time.


**Proposal 2: **
Considering the riverflow values of those above 75 cumecs and then subtracting $75$ from it because our location parameter is given to be 0, which usually is defined by the minimum value in the distribution. And so we are trying to model how much the river flow exceeded $75$. To investigate this proposal, we will plot a Q-Q plot as well as a normalised histogram of the data with the theoretical pdf. These two plots will help us graphically determine if the riverflow in excess of 75 cumecs can be modeled using the GPD defined.

```{r}
#| echo: false
threshold <- 75
proposal_2_data <- river_data[river_data$flow > threshold, ]
proposal_2_data$flow <- proposal_2_data$flow - 75
```
...
#| warning: false
#| echo: false
#| fig-cap: empirical data histogram and theoretical pdf for river flow in excess of 75
#| label: fig-gpd-emp-hist-pdf

# function for the density of gpd

dGPD <- function(x, sigma, xi, u) {
  if (sigma <= 0) stop("sigma must be positive")

  # For xi ≠ 0
  if (xi != 0) {
    y <- 1 + (xi * (x - u)) / sigma
    y[y < 0] <- 0  # the max condition
    pdf <- (1 / sigma) * (y)^(-1/xi - 1)
    pdf[y <= 0] <- 0  
  } else {
    # For xi = 0 
    pdf <- (1 / sigma) * exp(-(x - u) / sigma)
    pdf[x < u] <- 0 
  }
...
x_empirical <- proposal_2_data$flow
...
hist(x_empirical,
     ...
     col = "skyblue",
     border = "white",
     main = "Empirical Data vs Theoretical GPD PDF",
     xlab = "Flow")

# Overlay theoretical GPD PDF
x_vals <- seq(min(x_empirical), max(x_empirical), length.out = 300)
lines(x_vals, dGPD(x_vals, sigma = 29.7, xi = 0.62, u = 0),
      col = "red", lwd = 2)

legend("topright",
       legend = c("Empirical Density", "Theoretical GPD PDF"),
       col = c("skyblue", "red"),
       lwd = c(10, 2),
       bty = "n")
```
...
Like we did in task 3, we are using Q-Q plot, in @fig-gpd-qqplot-q4, to assist with telling whether observed data matches the theoretical distribution. As mentioned earlier, we are only working with the values over 75, and then subtracting 75 from them. The data points in @fig-gpd-qqplot-q4, lie very close to the line $y= x$, which shows that the excess river flow values agree with the theoretical GPD distribution suggested by the hydrologist. However, a few points in the upper tail deviate above the line, they are more extreme than the theoretical model expected. In @fig-gpd-emp-hist-pdf, a histogram of the empirical exceedances was plotted along with the theoretical GPD probability density function. The histogram is heavily right skewed with alot of value close to zero. The histogram also shows a long upper tail. The theoretical GPD pdf matches the shape of the histogram fairly closely. It does a good job of matching the right skew as well as the long upper tail.

Overall, the two figures, @fig-gpd-qqplot-q4 and @fig-gpd-emp-hist-pdf, provide a good argument to support the hydrologist's proposal that for river flows exceeding 75 cumecs, it is appropriate to model the data as GPD($\sigma =29.7$, $\xi=0.62$, $u = 0$). (We have not explicitly tested for the Independent and identically distributed part of the proposal).
