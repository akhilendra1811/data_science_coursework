...
**Proposal 2: **
Considering the riverflow values of those only above 75 and then subtracting $75$ from it because our location parameter is given to be 0, which usually is defined by the minimum value in the distribution. and so we are trying to model how much the river flow exceeded $75$.
...
Overall, the two figures, @fig-gpd-qqplot-q4 and @fig-gpd-emp-hist-pdf, provide a good argument to support the hydrologist's proposal that for river flows exceeding 75 cumecs, it is appropriate to model the data as independent and identically distributed GPD($\sigma =29.7$, $\xi=0.62$, $u = 0$).
